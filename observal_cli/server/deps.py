...
def is_installed(service: str) -> bool:
    ...
    elif service == "legacy_clickhouse":
        return bins["legacy_clickhouse"].exists()
    elif service == "redis":
...
def install_single(service: str, force: bool = False) -> None:
    ...
    Args:
        service: One of "postgres", "redis", or "legacy_clickhouse" (cutover only).
        force: Re-download even if already installed.
    """
    if not force and is_installed(service):
        console.print(f"[green]\u2713[/green] {service} already installed")
    ...
    BIN_DIR.mkdir(parents=True, exist_ok=True)
    urls = get_dep_urls()
    if service == "legacy_clickhouse":
        from observal_cli.server.constants import get_legacy_clickhouse_url

        urls = {service: get_legacy_clickhouse_url()}
    checksums = _fetch_checksums()
