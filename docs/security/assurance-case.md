...
Relevant actors include unauthenticated internet clients, malicious authenticated users, compromised user tokens, hostile registry submissions, compromised dependencies, and network attackers between clients and a remote deployment. Administrators and host operators are trusted to protect the host, secret files, backups, and TLS termination. PostgreSQL, the telemetry store, Redis, the API, and workers are trusted only inside the deployment network.
...
4. **Application to data services:** The API and workers cross the container network into PostgreSQL, the telemetry store (bearer-token HTTP), and Redis. Database ports bind to loopback in the server package by default.
