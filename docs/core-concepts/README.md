...
The registry stores users, agents, components, review state, and alert rules in PostgreSQL. Session events and aggregates are stored separately in the DuckDB telemetry store. See [Session tracking and reconciliation](session-tracking.md) for the complete session data flow.
