...
Every service should show `healthy` or `running`. The API waits for Postgres, the telemetry store, and Redis to pass health checks before starting. Expect 15–30 seconds on first boot.
