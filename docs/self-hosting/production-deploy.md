...
flowchart TB
    ...
    ch["Telemetry store (DuckDB) - EBS gp3"]
    grafana[Grafana]
...
| Telemetry store | EC2 + EBS gp3 | Single-writer DuckDB service; daily snapshot to S3 |
...
flowchart TB
    ...
    ch["Telemetry store (DuckDB) - Persistent Disk"]
    grafana[Grafana]
    prometheus[Prometheus]
...
| Telemetry store | GCE instance | Docker Compose on a single VM; daily snapshot to GCS |
...
The telemetry store is deliberately a single writer on one host. Availability comes from the persistent volume (EBS / persistent disk survives instance replacement), the daily object-storage snapshot, and the ingest clients' local outbox: while the store is unreachable the API returns `503 telemetry_unavailable`, the registry keeps working, and harness hooks retry delivery, so no session data is lost.
...
- [ ] Restore-test a telemetry snapshot
