...
| Large team (50+) | 8+ vCPU | 32 GB | 500 GB + fast SSD | High telemetry volume; give the telemetry store its own host and fast SSD |
...
| `observal-web` | 256 MB |
| `observal-telemetry` | 2 GB |
| `observal-redis` | 256 MB |
| `observal-grafana` | 512 MB |

The telemetry store is the memory-hungry one. Its DuckDB ceiling (`TELEMETRY_MEMORY_LIMIT`, default 1536 MB) should stay ~25% below the container limit; 2 GB is sufficient for the supported 30-million-event target.

## Disk: where the data goes

The heaviest user of disk is the **telemetry store** (`tdata` volume). Growth depends on:
...
* **Host bindings**: new server-package installs bind nginx, the direct web port, PostgreSQL, the telemetry store, Redis, Prometheus, and Grafana to `127.0.0.1` by default.
