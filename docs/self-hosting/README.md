...
flowchart TB
    ...
    ch["observal-telemetry - DuckDB telemetry store"]
...
2. **Protect database passwords**: server-package setup generates restricted PostgreSQL credential files and a telemetry token; source deployments must replace the `.env.example` defaults.
