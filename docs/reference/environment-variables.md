...
| `TELEMETRY_URL`       | `http://localhost:8125`                                          | Telemetry store (DuckDB service) base URL. Supports `TELEMETRY_URL_FILE`. |
| `TELEMETRY_TOKEN`     | empty                                                            | Bearer token shared by the API, worker, Grafana, and the telemetry service. Supports `TELEMETRY_TOKEN_FILE`. Required outside tests. |
...
| `DATA_RETENTION_DAYS` | `90`                                                             | Session event retention in days, applied by the worker purge job. `0` disables. Minimum non-zero: `7` |
...
| `CLICKHOUSE_HOST_PORT` | `8123`  | Legacy ClickHouse (profile only) |
...
| `GRAFANA_TELEMETRY_TOKEN_FILE` | generated file in server package | Telemetry token file mounted only into Grafana (Infinity datasource) |
