...
| **Source Compose** (`docker/`) | Uses a dedicated `Dockerfile.telemetry` image and keeps `docker/clickhouse/` only for the legacy profile; API/worker/init images exclude DuckDB. |
...
* `docker/Dockerfile.telemetry`: installs the isolated telemetry dependency group; `Dockerfile.api` contains migration-time Arrow support but no DuckDB engine.
