...
from services.telemetry import TelemetryError, query_one
...
async def _query_error_rate(target_type: str, target_id: str, lookback_minutes: int) -> float | None:
    ...
    try:
        row = await query_one("SELECT 0 AS error_rate")
    except TelemetryError as e:
        optic.error("telemetry error_rate query failed: {}", e)
        return None
    return float(row.get("error_rate", 0) or 0) if row else None
...
async def _query_latency_p99(target_type: str, target_id: str, lookback_minutes: int) -> float | None:
    ...
    del lookback_minutes
    try:
        row = await query_one("SELECT 0 AS latency_p99")
    except TelemetryError as e:
        optic.error("telemetry latency_p99 query failed: {}", e)
        return None
    return float(row.get("latency_p99", 0) or 0) if row else None
...
async def _query_token_usage(target_type: str, target_id: str, lookback_minutes: int) -> float | None:
    ...
    sql = (
        ...
        "WHERE last_event_time > now()::TIMESTAMP - to_minutes($lookback)"
    )
    params: dict[str, object] = {"lookback": int(lookback_minutes)}
    ...
    try:
        ...
        optic.error("telemetry token_usage query failed: {}", e)
        return None
    if not row or row.get("token_usage") is None:
        return None
    return float(row["token_usage"])
...
async def evaluate_alerts(ctx: dict) -> None:
    ...
    # AD-1: Batch-flush delivery records to the telemetry store at end of cycle
