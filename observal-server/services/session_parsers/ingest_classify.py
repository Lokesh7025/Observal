...
def _ts_claude_code(parsed: dict) -> str | None:
    """Return telemetry timestamp string from a Claude Code JSONL line, or None."""
...
def _ts_kiro(parsed: dict) -> str | None:
    """Return telemetry timestamp string from a Kiro JSONL line, or None.
...
def _ts_cursor(parsed: dict) -> str | None:
    """Return telemetry timestamp string from a Cursor JSONL line, or None.
...
def _ts_pi(parsed: dict) -> str | None:
    """Return telemetry timestamp string from a Pi JSONL line, or None.
...
def _ts_copilot_cli(parsed: dict) -> str | None:
    """Return telemetry timestamp string from a Copilot CLI JSONL line, or None."""
    raw = parsed.get("ts")
    if not raw:
        event = parsed.get("event")
        if isinstance(event, dict):
...
def _ts_antigravity(parsed: dict) -> str | None:
    """Return telemetry timestamp string from an Antigravity transcript line."""
...
def extract_timestamp(harness: str, parsed: dict) -> str | None:
    """Return a telemetry-formatted timestamp string for a JSONL line, or None.
...
def get_extra_rows(
    ...
    """Return any extra telemetry rows to insert after the main ingest loop.
