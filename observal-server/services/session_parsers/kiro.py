...
def parse_rows(rows: list[dict]) -> list[dict]:
    ...
    Each telemetry row contains a ``raw_line`` field holding one line of the
    ...
    for row in rows:
        ...
        jsonl_ts = _epoch_to_timestamp(epoch_s) if epoch_s else None
...
def _epoch_to_timestamp(epoch_s: int | float) -> str | None:
    """Convert unix epoch seconds to a telemetry datetime string.
