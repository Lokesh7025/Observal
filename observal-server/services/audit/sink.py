...
"""Loguru audit sink: buffers records and batch-inserts to the telemetry store."""
...
async def audit_sink(message: str) -> None:
    ...
    # Format timestamp as YYYY-MM-DD HH:MM:SS.mmm (no timezone suffix)
...
async def _flush() -> None:
    """Flush the buffer to the telemetry store. Must be called under _buffer_lock."""
    if not _buffer:
        return
    batch = list(_buffer)
    _buffer.clear()
