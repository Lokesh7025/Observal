...
async def _session_metrics() -> tuple[dict[str, int], dict[str, int | float], dict[str, int]]:
    ...
    try:
        rows = await tq(
            ...
            "count(DISTINCT session_id) FILTER (WHERE last_event_time >= now()::TIMESTAMP - INTERVAL 7 DAY) AS sessions_7d, "
            "count(DISTINCT session_id) FILTER (WHERE last_event_time >= now()::TIMESTAMP - INTERVAL 30 DAY) AS sessions_30d, "
            "count(DISTINCT user_id) FILTER (WHERE user_id != '' AND last_event_time >= now()::TIMESTAMP - INTERVAL 7 DAY) AS active_users_7d, "
            "count(DISTINCT user_id) FILTER (WHERE user_id != '' AND last_event_time >= now()::TIMESTAMP - INTERVAL 30 DAY) AS active_users_30d, "
            "count(DISTINCT agent_id) FILTER (WHERE agent_id != '' AND last_event_time >= now()::TIMESTAMP - INTERVAL 7 DAY) AS active_agents_7d, "
            "count(DISTINCT agent_id) FILTER (WHERE agent_id != '' AND last_event_time >= now()::TIMESTAMP - INTERVAL 30 DAY) AS active_agents_30d, "
            "sum(event_count) FILTER (WHERE last_event_time >= now()::TIMESTAMP - INTERVAL 7 DAY) AS events_7d, "
            "sum(event_count) FILTER (WHERE last_event_time >= now()::TIMESTAMP - INTERVAL 30 DAY) AS events_30d, "
            "sum(prompt_count) FILTER (WHERE last_event_time >= now()::TIMESTAMP - INTERVAL 7 DAY) AS prompts_7d, "
            "sum(prompt_count) FILTER (WHERE last_event_time >= now()::TIMESTAMP - INTERVAL 30 DAY) AS prompts_30d, "
            "sum(tool_call_count) FILTER (WHERE last_event_time >= now()::TIMESTAMP - INTERVAL 7 DAY) AS tool_calls_7d, "
            "sum(tool_call_count) FILTER (WHERE last_event_time >= now()::TIMESTAMP - INTERVAL 30 DAY) AS tool_calls_30d, "
            "sum(tool_result_count) FILTER (WHERE last_event_time >= now()::TIMESTAMP - INTERVAL 7 DAY) AS tool_results_7d, "
            "sum(tool_result_count) FILTER (WHERE last_event_time >= now()::TIMESTAMP - INTERVAL 30 DAY) AS tool_results_30d, "
            "sum(input_tokens) FILTER (WHERE last_event_time >= now()::TIMESTAMP - INTERVAL 7 DAY) AS input_tokens_7d, "
            "sum(input_tokens) FILTER (WHERE last_event_time >= now()::TIMESTAMP - INTERVAL 30 DAY) AS input_tokens_30d, "
            "sum(output_tokens) FILTER (WHERE last_event_time >= now()::TIMESTAMP - INTERVAL 7 DAY) AS output_tokens_7d, "
            "sum(output_tokens) FILTER (WHERE last_event_time >= now()::TIMESTAMP - INTERVAL 30 DAY) AS output_tokens_30d, "
            "sum(cache_read_tokens) FILTER (WHERE last_event_time >= now()::TIMESTAMP - INTERVAL 7 DAY) AS cache_read_tokens_7d, "
            "sum(cache_read_tokens) FILTER (WHERE last_event_time >= now()::TIMESTAMP - INTERVAL 30 DAY) AS cache_read_tokens_30d, "
            "sum(cache_write_tokens) FILTER (WHERE last_event_time >= now()::TIMESTAMP - INTERVAL 7 DAY) AS cache_write_tokens_7d, "
            "sum(cache_write_tokens) FILTER (WHERE last_event_time >= now()::TIMESTAMP - INTERVAL 30 DAY) AS cache_write_tokens_30d, "
            "sum(total_credits) FILTER (WHERE last_event_time >= now()::TIMESTAMP - INTERVAL 7 DAY) AS credits_7d, "
            "sum(total_credits) FILTER (WHERE last_event_time >= now()::TIMESTAMP - INTERVAL 30 DAY) AS credits_30d, "
            "sum(greatest(date_diff('second', first_event_time, last_event_time), 0)) FILTER (WHERE "
            "last_event_time >= now()::TIMESTAMP - INTERVAL 30 DAY) AS session_duration_seconds_30d, "
            "count(*) FILTER (WHERE last_event_time >= now()::TIMESTAMP - INTERVAL 30 DAY AND tool_call_count > 0) AS sessions_with_tools_30d, "
            "count(*) FILTER (WHERE last_event_time >= now()::TIMESTAMP - INTERVAL 30 DAY AND input_tokens + output_tokens > 0) "
            "AS sessions_with_tokens_30d, "
            "count(*) FILTER (WHERE last_event_time >= now()::TIMESTAMP - INTERVAL 30 DAY AND agent_id != '') "
            "AS registered_agent_sessions_30d, "
            "count(*) FILTER (WHERE last_event_time >= now()::TIMESTAMP - INTERVAL 30 DAY AND agent_id = '') "
            "AS unregistered_agent_sessions_30d, "
            "count(*) FILTER (WHERE last_event_time >= now()::TIMESTAMP - INTERVAL 30 DAY AND parent_session_id = '') "
            "AS top_level_sessions_30d, "
            "count(*) FILTER (WHERE last_event_time >= now()::TIMESTAMP - INTERVAL 30 DAY AND parent_session_id != '') "
            "AS subagent_sessions_30d, "
            "count(DISTINCT agent_version) FILTER (WHERE agent_version != '' AND last_event_time >= now()::TIMESTAMP - INTERVAL 30 DAY) "
            "AS distinct_agent_versions_30d, "
            "count(DISTINCT model) FILTER (WHERE model != '' AND last_event_time >= now()::TIMESTAMP - INTERVAL 30 DAY) "
    ...
    try:
        rows = await tq(
            ...
            "FROM session_events WHERE timestamp >= now()::TIMESTAMP - INTERVAL 30 DAY"
        )
        if rows:
