...
async def _telemetry_rows(sql: str, params: dict | None = None) -> list[dict]:
    """Run a read-only telemetry query scoped to the deployment project.

    Raises ``TelemetryError`` on failure; the app-level handler turns it into
    a 503 so dashboards never render zeros for an outage.
    """
    bound = {"pid": DEFAULT_PROJECT_ID}
    if params:
        bound.update(params)
    return await tq(sql, bound)
...
async def overview_stats(
    ...
    # Fan out all independent queries in parallel (3 Postgres + 2 telemetry)
    total_mcps_stmt = (
