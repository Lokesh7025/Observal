...
async def _resolve_telemetry_conn() -> TelemetryConnParams:
    """Build TelemetryConnParams from boot config."""
    from config import settings

    return TelemetryConnParams(url=settings.TELEMETRY_URL, token=settings.TELEMETRY_TOKEN)
...
async def run_migration_job(ctx: dict, job_id: str) -> None:
    ...
    telemetry_conn = await _resolve_telemetry_conn()
        ...
        async with asyncio.timeout(timeout_seconds):
            if operation_type == MigrationOperation.export:
                result_json, artifacts_json, schema_version = await _run_export(
                    data_scope, pg_conn, telemetry_conn, artifact_dir, reporter
                )
...
async def _run_export(
    ...
    telemetry_conn: TelemetryConnParams,
    artifact_dir: str,
    ...
    if data_scope in (MigrationScope.telemetry, MigrationScope.both):
        ...
        telemetry_result = await export_telemetry(
            telemetry_conn,
            telemetry_output_dir,
            reporter,
            migration_id=Path(artifact_dir).name,
        )
        result["telemetry_size_bytes"] = telemetry_result.total_size_bytes
        result["telemetry_rows"] = telemetry_result.total_rows
        ...
        with _tarfile.open(telemetry_archive_path, "w:gz") as tar:
            for path in sorted(telemetry_output_dir.rglob("*")):
                if path.is_file():
                    tar.add(str(path), arcname=str(path.relative_to(telemetry_output_dir)))
...
async def _run_import(
    ...
    if data_scope in (MigrationScope.telemetry, MigrationScope.both):
        ...
        telemetry_result = await import_telemetry(
            telemetry_conn,
            telemetry_dir,
            reporter,
        )
        for table, count in (telemetry_result.tables_imported or {}).items():
            result["rows_inserted"][table] = result["rows_inserted"].get(table, 0) + count
        result["derived_rebuild"] = telemetry_result.derived_rebuild
...
async def _run_validate(
    ...
    if data_scope in (MigrationScope.telemetry, MigrationScope.both):
        ...
        telemetry_val = await validate_telemetry(
            telemetry_conn,
            pg_conn,
            telemetry_dir,
            reporter,
