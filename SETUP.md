...
| **RAM**           | 4 GB+                 | The telemetry store is the memory consumer. 6 GB recommended for comfortable use.                                                                                                                                                                 |
...
> **Before a real deployment from source:** change `SECRET_KEY`, `POSTGRES_PASSWORD`, `TELEMETRY_TOKEN`, and unset all `DEMO_*` variables. Server-package installs generate these credentials as files automatically. See the [Self-hosting overview](docs/self-hosting/README.md), [Configuration](docs/self-hosting/configuration.md), [Databases](docs/self-hosting/databases.md), and [Upgrades](docs/self-hosting/upgrades.md).
...
| `observal-telemetry`  | `localhost:8125`        | Telemetry store (session and audit events) |
...
All services except `observal-init` (which exits after migrations) should show `healthy` or `running`. The API waits for Postgres, the telemetry store, and Redis before starting. Allow 15–30 seconds on first boot.
...
| `TELEMETRY_HOST_PORT` | `8125`   | Telemetry store |
