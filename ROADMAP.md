...
Harden `observal server upgrade` and explicit rollback around a coherent backup of PostgreSQL, API and JWT keys, telemetry store data, and image and config state. Pull and recreate target containers, apply migrations, run health and smoke checks, and restore all previous state automatically on any failure. Do not add a remote arbitrary-execution control plane.
