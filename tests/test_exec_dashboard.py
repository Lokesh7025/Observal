...
def ch_mock(monkeypatch, *responses):
    ...
    monkeypatch.setattr(dashboard, "_telemetry_rows", mock)
    return mock
...
async def test_adoption_aggregates_months_current_users_and_departments(monkeypatch):
    ...
    assert all("project_id = $pid" in call.args[0] for call in ch.await_args_list)
...
async def test_postgres_and_clickhouse_failures_are_not_silently_hidden(monkeypatch):
    ...
    monkeypatch.setattr(dashboard, "_telemetry_rows", AsyncMock(side_effect=RuntimeError("clickhouse unavailable")))
