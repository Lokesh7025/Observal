...
@pytest.mark.asyncio
async def test_telemetry_rows_binds_project_and_passes_params(monkeypatch):
    query = AsyncMock(return_value=[{"cnt": 3}])
    monkeypatch.setattr(dashboard, "tq", query)
    sql = "SELECT count(*) AS cnt FROM session_stats_agg WHERE project_id = $pid"
    params = {"days": 7}

    result = await dashboard._telemetry_rows(sql, params)

    assert result == [{"cnt": 3}]
    query.assert_awaited_once_with(sql, {"pid": "default", "days": 7})
    assert params == {"days": 7}


@pytest.mark.asyncio
async def test_telemetry_rows_propagates_store_failures(monkeypatch):
    from services.telemetry import TelemetryUnavailableError

    monkeypatch.setattr(dashboard, "tq", AsyncMock(side_effect=TelemetryUnavailableError("down")))

    with pytest.raises(TelemetryUnavailableError):
        await dashboard._telemetry_rows("SELECT 1")


@pytest.mark.asyncio
async def test_overview_stats_aggregates_postgres_and_telemetry_with_exact_queries(monkeypatch):
    db = _db(scalar_values=[4, 3, 9])
    ch = AsyncMock(side_effect=[[{"cnt": "17"}], [{"cnt": 6}]])
    monkeypatch.setattr(dashboard, "_telemetry_rows", ch)
...
async def test_overview_stats_returns_zeroes_for_empty_aggregates(monkeypatch):
    ...
    monkeypatch.setattr(dashboard, "_telemetry_rows", AsyncMock(side_effect=[[], []]))
...
@pytest.mark.asyncio
async def test_overview_stats_exposes_null_telemetry_aggregate_as_invalid(monkeypatch):
    db = _db(scalar_values=[0, 0, 0])
    monkeypatch.setattr(dashboard, "_telemetry_rows", AsyncMock(side_effect=[[{"cnt": None}], []]))
