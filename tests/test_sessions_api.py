...
@pytest.mark.asyncio
@pytest.mark.parametrize(
    ("user", "expects_user_filter"),
    [
        (_user(), True),
        (_user(UserRole.admin, trace_privacy=False), False),
        (_user(UserRole.admin, trace_privacy=True), True),
        (_user(UserRole.super_admin, trace_privacy=True), False),
    ],
)
async def test_sessions_summary_applies_trace_visibility_and_transforms_counts(monkeypatch, user, expects_user_filter):
    query = AsyncMock(return_value=[{"total": 12, "today_sessions": 3}])
    monkeypatch.setattr(sessions, "tq", query)
    ...
    assert ("user_id = $uid" in sql) is expects_user_filter
    assert params == ({"uid": str(USER_ID)} if expects_user_filter else {})
...
async def test_sessions_stats_transforms_counts(monkeypatch, rows, expected):
    ...
    assert "session_stats_agg" in query.await_args.args[0]
...
async def test_session_detail_empty_identity_preserves_identifier_and_user_isolation(monkeypatch, session_id):
    ...
    # Summary lookup first, then the event table fallback - both user-scoped, both parameterised.
    assert query.await_count == 2
    for item in query.await_args_list:
        sql, params = item.args
...
async def test_admin_session_detail_uses_canonical_identity_for_both_event_queries(monkeypatch):
    identity = {"session_key": 42, "project_id": "project-a", "user_id": str(OTHER_USER_ID), "harness": "cursor"}
    ...
    identity_sql, identity_params = query.await_args_list[0].args
    assert "user_id = $uid" not in identity_sql and identity_params == {"sid": "shared-id"}
    for item in query.await_args_list[1:]:
        sql, params = item.args
        assert "session_events" in sql and "rendered" in sql
        assert params["key"] == 42 and params["sid"] == "shared-id"
        assert params["uid"] == str(OTHER_USER_ID) and params["harness"] == "cursor"
    assert "parent_session_key = $key" in query.await_args_list[2].args[0]


@pytest.mark.asyncio
async def test_incremental_session_detail_uses_offset_for_parent_and_subagents(monkeypatch):
    identity = {"session_key": 7, "project_id": "project-a", "user_id": str(USER_ID), "harness": "claude-code"}
    query = AsyncMock(side_effect=[[identity], [], []])
    monkeypatch.setattr(sessions, "tq", query)
    ...
    for item in query.await_args_list[1:]:
        sql, params = item.args
        assert "line_offset::BIGINT > $offset" in sql
        assert params["offset"] == 7


@pytest.mark.asyncio
async def test_session_detail_parses_events_subagents_and_agent_name(monkeypatch):
    identity = {"session_key": 1, "project_id": "project-a", "user_id": str(USER_ID), "harness": "kiro"}
    parent_rows = [
        {
            "line_offset": "2",
            "harness": "kiro",
...
async def test_session_detail_without_agent_uses_default_harness_and_no_database(monkeypatch):
    identity = {"session_key": 1, "project_id": "project-a", "user_id": str(USER_ID), "harness": "claude-code"}
...
async def test_session_detail_agent_resolution_failures_do_not_hide_events(monkeypatch, agent_id, db_error):
    identity = {"session_key": 1, "project_id": "project-a", "user_id": str(USER_ID), "harness": "cursor"}
...
async def test_session_parser_failure_propagates(monkeypatch):
    ...
    monkeypatch.setattr(sessions, "tq", AsyncMock(side_effect=[[identity], rows, []]))
...
async def test_bind_session_agent_checks_owner_and_sets_expiring_binding(monkeypatch):
    ...
    monkeypatch.setattr(sessions, "tq", ownership)
...
async def test_binding_reports_redis_unavailability(monkeypatch, failure_at):
    ...
    monkeypatch.setattr(sessions, "tq", AsyncMock())
    monkeypatch.setattr("services.redis.get_redis", get_redis)
