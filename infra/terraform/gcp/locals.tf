...
locals {
  ...
  telemetry_url  = "http://${google_compute_instance.data_host.network_interface[0].network_ip}:8125"
  clickhouse_url = var.enable_legacy_clickhouse ? "clickhouse://default:${random_password.clickhouse.result}@${google_compute_instance.data_host.network_interface[0].network_ip}:8123/observal" : ""
}
