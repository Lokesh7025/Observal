...
locals {
  ...
  # Internal DNS names on the data host (telemetry store; legacy ClickHouse during cutover only).
  telemetry_host_internal  = "telemetry.${var.internal_dns_zone}"
  telemetry_url_internal   = "http://${local.telemetry_host_internal}:8125"
  clickhouse_host_internal = var.enable_legacy_clickhouse ? "clickhouse.${var.internal_dns_zone}" : ""
