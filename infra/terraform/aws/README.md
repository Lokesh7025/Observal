...
The stateless app tier (api/web/worker) lives on Fargate across both AZs with autoscaling and rolling deploys. The stateful data tier lives on a single EC2 with EBS so the single-writer DuckDB telemetry store keeps its database file across instance replacements; daily snapshots go to S3. Prometheus and Grafana are enabled with `observability_stack`.
...
5. **(Upgrades only) `enable_legacy_clickhouse = true`** keeps the old ClickHouse container on the data host while `observal server migrate telemetry-cutover` back-fills the telemetry store; set it back to `false` afterwards.
...
- [ ] Verify the daily telemetry snapshot lands in the backups bucket and restore-test it
