...
locals {
  ...
  # The data VM always exists: it hosts the single-writer telemetry store
  # (plus Redis and Prometheus when those are self-hosted).
  needs_vm = true
  ...
  telemetry_url     = "http://${azurerm_network_interface.clickhouse[0].private_ip_address}:8125"
  clickhouse_url    = var.enable_legacy_clickhouse ? "clickhouse://default:${random_password.clickhouse.result}@${azurerm_network_interface.clickhouse[0].private_ip_address}:8123/observal" : ""
